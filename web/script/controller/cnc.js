define([
	...
	'../controller/cnc/cncFileTransfer',
	'../controller/cnc/cncOffset',
], function ( app, highcharts, cncOOE, cncRecord, cncAlarm, cncDiagnosis, cncFileTransfer, cncOffset ) {
	app.controller('cncControl', function(
		$scope,$http,$interval,$timeout,
		$rootScope,$routeParams,
		MODE,
		accountMgr,frontendAdaptor,commandMgr,cncDataMgr,versionMgr,fileMgr
		){
		...
		$scope.tabHisClick = function( tabName, tabData )
		{
			...
			$scope.showDataHtml = "view/web/content/cnc/" + tabName + ".html";
		...
		{
			//get agent version
			fnInitCncAgentVersion();

			//default show first item
			$scope.GetCncData();
			InitCNCFunctionGroup();

			//after get data, assign function
			$timeout(function(){
				//TODO: cnc provide function list is based on cncData, so it needs to wait cncData already getten
				//now use delay 1.5s to load function list, bad
				$scope.ShowFunctionGroupItem( $scope.cncFunctionGroups[0] );
			},1500);
		}
		fnInitCncAgentVersion = function()
		{
			this.fnGetResult = function(response){
				...
				$scope.szVersion['AGENT'] = response.data.agent_version;
				$scope.szVersion['API'] = response.data.api_version;
			}

			var objCncAgent = { "cncID":$scope.cncID };
			frontendAdaptor.fnGetResponse( 'CNCDATA', 'GetCncAgentVersion', objCncAgent, this.fnGetResult, false );
		}
		...
		//version comparsion
		$scope.fnCheckAgentVersion = function( szFeatureClass, szFeature )
		{
			if( $scope.szVersion['AGENT'] == "-1" ){
				return false;
			}

			//default, use to confirm agent version
			$rootScope.szSupportVersion = new Object();
			$rootScope.szSupportVersion['AGENT'] = "-1";
			$rootScope.szSupportVersion['API'] = $scope.szVersion['API'];
			
			return versionMgr.fnIsAgentSupport(MODE,$scope.szVersion['AGENT'],szFeatureClass,szFeature);
		}

		$scope.fnDownloadAgentFile = function()
		{
			fileMgr.fnDownloadFile( {'szAgnetVersion':$rootScope.szSupportVersion['AGENT'], 'szAPIVersion':$rootScope.szSupportVersion['API']},'AgentFile' );
		}
	});
	...
	app.controller( 'cncFileTransfer', cncFileTransfer );
	app.controller( 'cncOffset', cncOffset );
});
